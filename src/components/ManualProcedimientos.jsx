...
/**
 * Dibuja el bloque de firmas oficial de 3 columnas exactamente como la plantilla SGC
 */
...
function addPage(doc, counters) {
    // Dibujar firmas en el pie de la página saliente
...
function noteBox(doc, text, y) {
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(30, 87, 153);
    doc.text('[NOTA / OBSERVACIÓN OPERATIVA]', ML + 4, y + 4.5);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(0, 0, 0);
...
export async function generateManualPDF() {
    ...
    y = para(doc, 'Definir los pasos, pautas y controles operativos para la correcta administración, gestión de cirugías, control de altas administrativas, trazabilidad de historias clínicas, cola de turnos y facturación dentro del Sistema de Admisión Quirúrgica (ADM-QUI) del Sanatorio Argentino. Este instructivo garantiza la estandarización de procesos conforme a los requerimientos de acreditación hospitalaria ITAES y normativas de calidad vigentes.', y);

    // ── 2. CAMPO DE APLICACIÓN ──
    y += 1;
    y = sectionTitle(doc, '2. CAMPO DE APLICACIÓN:', y);
    y = para(doc, 'El presente instructivo se aplicará al personal de Innovación y Transformación Digital, Admisión Quirúrgica, Recepción Central, Control de Altas Administrativas, Facturación Internado, Quirófano, Auditoría Médica y todo usuario de salud que opere el Sistema ADM-QUI en sus tareas diarias.', y);
    ...
    y = para(doc, 'Centraliza la programación quirúrgica extraída de SALUS. Permite enviar mensajes automáticos de preparación prequirúrgica por WhatsApp mediante un pipeline de estados codificado por colores:', y);
    y = bulletList(doc, [
    ...
    y = para(doc, 'Gestiona el flujo de recepción y atención al público:', y);
    y = bulletList(doc, [
    ...
    y = para(doc, 'Control y auditoría de todas las admisiones hospitalarias del Sanatorio Argentino:', y);
    y = bulletList(doc, [
    ...
    y = para(doc, 'Recepción de expedientes transferidos desde Altas Adm para su liquidación final. Permite marcar como Facturada o generar remito de Devolución con motivo específico hacia Control de Altas.', y);

    // 5.5 Asistente Beto IA
    y = checkPageBreak(doc, counters, y, 25);
    y = subTitle(doc, '5.5 Asistente Virtual Beto IA (BetoWidget / BetoPanel)', y);
    y = para(doc, 'Beto es el asistente de inteligencia artificial exclusivo de ADM-QUI. Capacidades principales:', y);
    y = bulletList(doc, [
    ...
    y = para(doc, 'Ante contingencias técnicas, el personal debe aplicar las siguientes pautas inmediatas:', y);
    autoTable(doc, {
        startY: y,
        margin: { left: 14, right: 14 },
    ...
    // Dibujar bloque de firmas oficial en el pie de la última página
    drawSignatures(doc, 254);
    ...
    return (
        <div style={{
            ...
            <div style={{
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td style={{ border: '1.5px solid #000', width: '25%', padding: '8px', textAlign: 'center', verticalAlign: 'middle' }}>
                                    ...
                                    <div style={{ fontSize: '10px', marginTop: '2px', color: '#4B5563' }}>Pág. 1 de 3</div>
                                </td>
                            </tr>
                ...
                {/* ── PIE DE DOCUMENTO CON TABLA DE FIRMAS (IGUAL A IMAGEN 2) ── */}
